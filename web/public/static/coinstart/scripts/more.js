...
$(function () {
    ...
    //РџСЂРё РІС‹Р±РѕСЂРµ РЅР°РїСЂР°РІР»РµРЅРёСЏ Рё РїР»Р°С‚С‘Р¶РЅРѕР№ СЃРёСЃС‚РµРјС‹
...
//РІС‹РІРѕРґ СЂРµС„РµСЂР°Р»СЊРЅРѕР№ СЃСѓРјРјС‹
function referalOutput() {
    ...
    if ( refsel == '5' ){
        if( /^([a-zA-Z0-9]{28,100})$/.test(to_requis1) == false ){
            ...
            $("#to_requis1").after("<div class='error-msg'>Р’РІРµРґРёС‚Рµ РєРѕС€РµР»С‘Рє Bitcoin</div>");
            return false;
        }
    }else if( refsel == '57' ){
        if( /^([a-zA-Z0-9]{3,100})$/.test(to_requis1) == false ){
            $("#to_requis1").addClass("error-input");
            $("#to_requis1").after("<div class='error-msg'>Р’РІРµРґРёС‚Рµ РєРѕС€РµР»С‘Рє Tether (ERC20)</div>");
            return false;
        }
    }else if( refsel == '55' ){
        if( /^([a-zA-Z0-9]{3,100})$/.test(to_requis1) == false ){
            $("#to_requis1").addClass("error-input");
            $("#to_requis1").after("<div class='error-msg'>Р’РІРµРґРёС‚Рµ РєРѕС€РµР»С‘Рє Tether (TRC20)</div>");
            return false;
        }
    }
...
function CreateReservePs(data){
    ...
    for ( _ in api.paySystems ) {
        ...
        if(res_id == 128) {
            data.reserves[res_id] = Number(2764.88)
        }

        if( $('#section-get .pay-select-li[data-pay-id=' + res_id +']').length && (res_curr !== "rub" && res_curr !== "uzs" && res_curr !== "uah")){
...
function createFromTo() {
    ...
    //РЅР°С…РѕРґРёРј РєСѓРґР° С…РѕС‚РёРј РїРµСЂРµРєР»СЋС‡РёС‚СЊСЃСЏ
...
function calculate(way) {
    ...
    if (way == 1) {
        var in_text = parseFloat($(".in-text").val()) || 0;
        $(".to-text").val(roundPs(in_text * course, to_bank.round));
    } else if (way == 2) {
        var to_text = parseFloat($(".to-text").val()) || 0;
...
function copyHandler() {
    $("a.copy-requisite").on("click", function () {
